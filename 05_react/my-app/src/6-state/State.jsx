...
export function Counter() {
  // count라는 상태와 이를 변경하는 setCount 함수
  // const를 사용하는 이유: 상태 참조 자체가 병경되지 않기 때문
  // setCount를 통해서 상태 변경이 이뤄짐
  ...
  return (
    <div>
      {/* JSX 주석문 */}
      <p>{count}</p>
      {/* 버튼 클릭 시 count를 1 증가시키는 이벤트 핸들러 */}
      <button
        onClick={() => {
          setCount(count + 1);
        }}
...
export function LikeButton() {
  ...
  return (
    <div>
      ...
      <p>{liked ? "이 게실물을 좋아합니다." : "좋아요!를 눌러주세요~"}</p>
    </div>
  );
}
